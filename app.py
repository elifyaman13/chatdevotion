...
def analyze_with_gpt(text, lang="tr"):
    prompt = (
        f"Analyze the following emotional message and offer a thoughtful, empathetic reflection in English:\n{text}"
        if lang == "en"
        else f"Aşağıdaki duygusal metni analiz et ve empatik, düşünceli bir psikolojik değerlendirme sun:\n{text}"
    )
...
def detect_emotion(text):
    keywords = {
        "motivation": ["motivasyon", "hedef", "başarı"],
        "sadness": ["üzgün", "hüzün", "keder", "depresyon"],
        "anxiety": ["anksiyete", "kaygı", "panik", "gerginlik"],
        "loneliness": ["yalnız", "yalnızlık", "boşluk", "izole"]
    }
    for emotion, keys in keywords.items():
        if any(word in text.lower() for word in keys):
            return emotion
    return "motivation"
...
def select_chat(chat_id):
    ...
    if chat_id in user["chat_sessions"]:
        user["chat_memory"] = user["chat_sessions"][chat_id].copy()
        user["active_session_id"] = chat_id
        user["gpt_analysis"] = None
    return redirect(url_for("index"))
